...
class Robo():
    def __init__(self, id, email=' ', password=' ', asset='EURUSD',value=1,minutos=1):
        self.email = email
     ...
     #  self.weight_of_indicators = self.get_weight_of_indicators()
        self.operations = int(input('Digite quantas operações o robo irá operar: '))
    ...
    def connection(self): #SUCESSO
        if self.email == ' ' and self.password == ' ':
            while True: 
                ...
                if check == True: print('Logado com sucesso!!');break
                else: print('Erro ao logar: ',reason)
        else:
            while True:
                Iq=IQ_Option(self.email,self.password)
                check, reason = Iq.connect()
    ...
    def get_indicators(self): # SUCESSO RETORNA (indicators_result, result)
        ...
        for indicator in indicators:
            if indicator['candle_size'] == (self.minutos * 60):
                indicators_result[indicator['name']] = indicator['action'] # hold, buy or sell
                if indicator['action'] == 'buy' : buy  += 1 # replace with indicator weights
                if indicator['action'] == 'sell': sell += 1 # replace with indicator weights
                if indicator['action'] == 'hold': hold += 1 # replace with indicator weights
        if hold/1  > buy+sell:
            result = 'hold'
        else:
            if buy > sell:
